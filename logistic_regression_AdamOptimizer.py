...
NUM_BATCHES = 7;
NUM_ITERATIONS = 5000;
LEARNING_RATE = [0.001]; #[0.005 0.001, 0.0001];
...
with tf.Session() as sess:
    ...
    for learning_rate in LEARNING_RATE:
        train_step = tf.train.AdamOptimizer(learning_rate).minimize(total_loss) #change the learning rate each time
    	tf.global_variables_initializer().run()
        for i in range(NUM_BATCHES*NUM_ITERATIONS):  
            ...
            if( i % NUM_BATCHES == 0): #everytime we reach 0, a new batch has started and therefore 1 epoch has been completed
                ...
                y_hat_new = sess.run(y_hat_new,feed_dict={x: np.transpose(trainX), y: trainTarget})
                print("Epoch:", epoch)
                ...
                #print(y_hat_new)
                #print("Y:")
                y_new = sess.run(y,feed_dict = {y: trainTarget})
